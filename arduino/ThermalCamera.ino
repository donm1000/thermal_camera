...
FLOATUNION_t udpPixels[COLS * ROWS];
...
void sendUdpPackets() {
      ...
      float testFloatValue = -40;
      FLOATUNION_t testFloat; 
      testFloat.number = testFloatValue;

      uint8_t buffer1[1025]; 
      uint8_t buffer2[1025];
      uint8_t buffer3[1025];
      ...
      for (int udpI = 0; udpI < 256; udpI++) {

        buffer1[bufferI] = udpPixels[udpI].bytes[0];
        bufferI++;
        buffer1[bufferI] = udpPixels[udpI].bytes[1];
        bufferI++;
        buffer1[bufferI] = udpPixels[udpI].bytes[2];
        bufferI++;
        buffer1[bufferI] = udpPixels[udpI].bytes[3];
        bufferI++;
      ...
      int ret = udp.endPacket();
        ...
        for (int udpI = 256; udpI < 512; udpI++) {

          buffer2[bufferI] = udpPixels[udpI].bytes[0];
          bufferI++;
          buffer2[bufferI] = udpPixels[udpI].bytes[1];
          bufferI++;
          buffer2[bufferI] = udpPixels[udpI].bytes[2];
          bufferI++;
          buffer2[bufferI] = udpPixels[udpI].bytes[3];
          bufferI++;
        ...
        for (int udpI = 512; udpI < 768; udpI++) {

          buffer3[bufferI] = udpPixels[udpI].bytes[0];
          bufferI++;
          buffer3[bufferI] = udpPixels[udpI].bytes[1];
          bufferI++;
          buffer3[bufferI] = udpPixels[udpI].bytes[2];
          bufferI++;
          buffer3[bufferI] = udpPixels[udpI].bytes[3];
          bufferI++;
    ...
    for (int x = 0; x < pixelsArraySize; x++) {
        if (x % COLS == 0)  {
            for (int j = 0 + x, k = (COLS - 1) + x; j < COLS + x; j++, k--) {
                ...
                udpPixels[j].number = reversePixels[k];
            }
        }
    }

    //Serial.println("Reverse Image completed");
...
void drawpixels(float *p, uint8_t rows, uint8_t cols) {
    ...
    currentF = (((get_point(p, rows, cols, cols / 2, rows / 2))*9)/5)+32;
    img.printf("%.2fF", currentF);
    img.pushSprite(0, 0);
